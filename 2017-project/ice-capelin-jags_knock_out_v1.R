...
##loop for knockout----
for(i in 1:15){
     model_data <- list(N2 = c(var_x, rep(NA, num_forecasts)), #var_x spacified in data section - ln_biomass_med
...
ggsave(paste0(filepath_gen, folder, "/credInt", insert_year[i], ".png"), width=10, height=8, units="in")
}
...
per_diff_file <- data.frame(year = integer(), 
                            ...
                            pi10 = numeric(), # 2.5% value of PI
                            pi90 = numeric(), # 2.5% value of PI
...
write_csv(per_diff_file, paste0(filepath_gen, "/sensitivity/per_diff_predictions.csv"))
...
write_csv(temp2, paste0(filepath_gen, "/sensitivity/predictions_CI_PI.csv"))
