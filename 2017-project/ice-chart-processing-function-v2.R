...
withinPolyAreaA <- function(x = data, sa = sa) {
  for (i in 1:nrow(x)) {
    if (is.na(x$SA[i])) {
      ...
      if (x$SA[i] %in% sa) {
        ...
        x$AREA_SA[i] <- 0 # if SA is not of desired value (but not NA) - then area is 0
      }
    }
  }
  return(x)
...
withinPolyAreaB <- function(x = data, sa = sa) {
  for (i in 1:nrow(x)) {
    if (is.na(x$SB[i])) {
      x$AREA_SB[i] <- 0  # if value of SA = NA, then set area to zero
    } else {
      if (x$SB[i] %in% sa) {
        x$AREA_SB[i] <- x$AREA[i] * as.numeric(x$CB[i])/10 # calculate area for desired values of SA
      } else {
        x$AREA_SB[i] <- 0 # if SA is not of desired value (but not NA) - then area is 0
      }
    }
  }
  return(x)
...
withinPolyAreaC <- function(x = data, sa = sa) {
  for (i in 1:nrow(x)) {
    if (is.na(x$SC[i])) {
      x$AREA_SC[i] <- 0  # if value of SA = NA, then set area to zero
    } else {
      if (x$SC[i] %in% sa) {
        x$AREA_SC[i] <- x$AREA[i] * as.numeric(x$CC[i])/10 # calculate area for desired values of SA
      } else {
        x$AREA_SC[i] <- 0 # if SA is not of desired value (but not NA) - then area is 0
      }
    }
  }
  return(x)
...
withinPolyAreaD <- function(x = data, sa = sa) {
  for (i in 1:nrow(x)) {
    if (is.na(x$SD[i])) {
      x$AREA_SD[i] <- 0  # if value of SA = NA, then set area to zero
    } else {
      if (x$SD[i] %in% sa) {
        x$AREA_SD[i] <- x$AREA[i] * as.numeric(x$CD[i])/10 # calculate area for desired values of SA
      } else {
        x$AREA_SD[i] <- 0 # if SA is not of desired value (but not NA) - then area is 0
      }
    }
  }
  return(x)
...
iceSubset <- function(x, ct = NULL, sa = NULL) {
  ...
  x@data <- withinPolyAreaA(x@data, sa = sa)
  x@data <- withinPolyAreaB(x@data, sa = sa)
  x@data <- withinPolyAreaC(x@data, sa = sa)
  x@data <- withinPolyAreaD(x@data, sa = sa)
  y <- subset(x, subset = CT %in% m2$ct & SA %in% m2$sa)
  return(y)
}
###########################################################
##' iceArea()-----------------------------
...
#'
#' @return x maintains the sub.egg data
#'          tice - the minimum latitude of the ice extent sum of the ice in the SPDF after the subsets
#' @export
#'
#' @examples
#' 
...
#iceTiming <- function(z){
  #minlat <- 
#    z %>%
#    tidy() %>%
#   slice(which.min(lat))
  #return(as.data.frame(minlat))  
#}
...
#' @examples
 calcAreaVolume <- function(y, ct = NULL, sa = NULL){ 
   areas <- volumes <- minlats <- minlongs <- rep(NA, length(y)) # create an empty object
   for(i in seq_along(y)) {
     ...
     if(all(is.na(ice$EGG_ATTR))) {  # START HERE
       ...
       if(is.null(sub.egg)) {
         ...
         if(class(sub.egg) != "try-error") {
           sub.egg1 <- iceSubset(sub.egg, ct = ct, sa = sa) # what shoudl this be?
           if(nrow(sub.egg1@data)==0){
             area <- volume <- 0
             minlat <- 55
             sub.egg1 <- sub.egg
           } else {
    ...
    #       sub.egg <- spTransform(sub.egg, CRS("+proj=lcc +lat_1=49 +lat_2=77 +lat_0=40 +lon_0=-100 +datum=NAD27"))
           a <- try(gArea(sub.egg1, byid = TRUE)) # sometimes holes are not identified correctly, so try and extract max polygon area within each id
           if(class(a) == "try-error") { 
             message("gArea didn't work. Trying alternate appriach")
             a <- sapply(slot(sub.egg1, "polygons"), function(x) max(sapply(slot(x, "Polygons"), slot, "area"))) 
           }
           if(class(a) != "try-error") {
             sub.egg1$AREA <- a * 1e-6 # replace polygon area (use square km)  
             sub.egg1 <- iceSubset(sub.egg1, ct = ct, sa = sa) # what shoudl this be?
             subarea <- iceArea(sub.egg1@data)
             area <- subarea[[2]]
             volume <- iceVolume(sub.egg1@data) 
