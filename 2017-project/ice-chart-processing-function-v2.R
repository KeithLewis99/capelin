...
#withinPolyAreaA <- function(x) {
#  for (i in 1:nrow(x$sub.egg1@data)) {
#   if (is.na(x$sub.egg1@data$SA[i])) {
#     x$sub.egg1@data$AREA_SA[i] <- 0  # if value of SA = NA, then set AREA_SA to zero
#   } else {
#     x$sub.egg1@data$AREA_SA[i] <- x$sub.egg1@data$AREA[i] * as.numeric(x$sub.egg1@data$CA[i])/10 # calculate area for desired values of SA
#    }
# }
# return(x)
#}
 ...
 calcAreaVolume <- function(y, ct = NULL, sa = NULL){ 
   ...
   for(i in seq_along(y)) {
     ...
     if(all(is.na(ice$EGG_ATTR))) {  # START HERE
       ...
       ## Apply filters
       attr.tab <- egg@data
       sub.egg <- try(gDifference(egg, filters, byid = TRUE)) # filters remove lots of polygons
