...
eggAttr.query <- function(x = data, ct = NULL, sa = NULL, sb = NULL){
  ...
  #x <- subset(x, subset = CT %in% ct & SA %in% sa & SB %in% sb | CT %in% ct & SA %in% sa)
...
trendsCalc <- function(x, y){
  ...
  if(class(y) != "try-error") {
    ...
    x@data$AREAice <- x@data$AREA*as.numeric(x@data$CT)
...
extractSPDFfinal <- function(z, i, ct = NULL, sa = NULL, sb = NULL){
  ...
  sub.egg1 @data$AREAice <- sub.egg1@data$AREA*as.numeric(sub.egg1@data$CT)
  out <- iceArea(sub.egg1@data)
  return(out)
}
