...
## data----
filepath_gen <- "Bayesian/biomass_2019_v1/" #"ag2_cond_ag1_DIC" "Bayesian/biomass_cond_ag1_DIC/" "Bayesian/biomass_cond_ag1_2_DIC_new/"
folder <- "sensitivity"
...
run_RM3_ <- rep(list(list()), 16)
y_pred_ <- rep(list(list()), 16)
y_med_ <- rep(list(list()), 16)
ci_df3_ <- rep(list(list()), 16)
p_med_ <- rep(list(list()), 16)
pi_df3_ <- rep(list(list()), 16)
err_df3_ <- rep(list(list()), 16)
err_new_df3_ <- rep(list(list()), 16)
err_mean_new_df3_ <- rep(list(list()), 16)
...
for(i in 1:16){
  message("i = ",i)
     ...
     err_df3_[[i]] <- insert_[[i]]$ln_biomass_med - y_pred_[, 16]
     err_new_df3_[[i]] <- insert_[[i]]$ln_biomass_med - y_new_[, 16]
     err_mean_new_df3_[[i]] <- insert_[[i]]$ln_biomass_med - mean(y_new_[, 16])
...
for(i in seq_along(2003:2018)){
     per_diff_file[i,1] <- insert_year[i]
     per_diff_file[i,3] <- p_med_[[i]][16]
     per_diff_file[i,4] <- pi_df3_[[i]][1, 16]
     per_diff_file[i,5] <- pi_df3_[[i]][2, 16]
     per_diff_file[i,6] <- pi_df3_[[i]][3, 16]
     per_diff_file[i,7] <- pi_df3_[[i]][4, 16]
}

paste0(filepath_gen, "/sensitivity/loo.pdf")
write_csv(per_diff_file, paste0(filepath_gen, "/sensitivity/per_diff_predictionsv2.csv"))
