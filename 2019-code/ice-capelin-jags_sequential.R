...
source("ice-capelin-covariates-FUN.R")
source("ice-capelin-jags_sequential-FUN.R")

#make the directory for the Bayesian analyses
if(!dir.exists("Bayesian")) dir.create("Bayesian")

# this creates the file path and directory for a specific run of the analysis.  Change the directory accordingly if changing the model run
folder_path1 <- "Bayesian/biomass_2019_v1/" #"Bayesian/biomass_cond_ag1_DIC/""Bayesian/ag2_cond_ag1_DIC/" "Bayesian/biomass_cond_ag1_2_DIC_new/"
if(!dir.exists(folder_path1)) dir.create(folder_path1)

if(!dir.exists(paste0(folder_path1, "recruitment_0"))) dir.create(paste0(folder_path1, "recruitment_0"))
if(!dir.exists(paste0(folder_path1, "recruitment_1"))) dir.create(paste0(folder_path1, "recruitment_1"))
if(!dir.exists(paste0(folder_path1, "recruitment_2"))) dir.create(paste0(folder_path1, "recruitment_2"))
if(!dir.exists(paste0(folder_path1, "mortality_0"))) dir.create(paste0(folder_path1, "mortality_0"))
if(!dir.exists(paste0(folder_path1, "mortality_1"))) dir.create(paste0(folder_path1, "mortality_1"))
if(!dir.exists(paste0(folder_path1, "mortality_2"))) dir.create(paste0(folder_path1, "mortality_2"))
if(!dir.exists(paste0(folder_path1, "rm_1"))) dir.create(paste0(folder_path1, "rm_1"))
if(!dir.exists(paste0(folder_path1, "rm_2"))) dir.create(paste0(folder_path1, "rm_2"))
if(!dir.exists(paste0(folder_path1, "rm_3"))) dir.create(paste0(folder_path1, "rm_3"))
if(!dir.exists(paste0(folder_path1, "sensitivity"))) dir.create(paste0(folder_path1, "sensitivity"))
...
# these help with if-else statements below
capelin_data_set <- "biomass" # type of capelin index - alt value ==  "biomass" "age"; in original analysis in 2017/2018, there was considerable debate over which one to use - we ended up using biomass
#cond_data_a1_2 <- "cond" # type of condition index - alt value == "resids" or "cond_dat_a1_2" - as above, there was consierable debate over what condition to use
dic_run <- "yes" # is this a DIC run? "yes" or "no"; this was added in case one wanted to run models for a longer time series; the DIC analysis is limited by the Bellevue Beach larval capelin index which started in 2001 (but lagged by 2 years = 2003)
...
STpred <- c(-0.454, -1.32)
PSpred <- c(-0.963, 0.753) #awating final value from P. Pepin
TIpred <- c(0.93, 0.57)  #first value estimated from this year (line 74-75), second, mean from centered data (line 168)

# set values for condition values
x <- "condition" #"resids"
y <- "a1_2" # "a1" - a1 = condition of age 1 capelin, a1_2 = age 1 & 2
...
# sets the subfolder
filepath_gen <- "biomass_2019_v1"  # for filepath for datasets and pairs Plots - "biomass_cond_ag1_DIC" "ag2_cond_ag1_DIC" "biomass_cond_ag1_2_DIC_new"

# changes of the axis and axis labels for credInt in graphs below
...
tice_day <- as.Date('2019-03-11') # this is the lowest ice extent as of 2018-04-17
...
#the below was for calculating an informative prior for the gamma distribution - we abandonded this for the paper because it added little to the analysis (plus it was real hard to explain)
# mean(ice1$tice/100)
# var(ice1$tice/100)
# 
# sd(ice1$tice/100)
...
#cond_dat <- read_csv('E:\\Capelin\\2019 capelin assessment\\Condition/condition_ag1_2_MF_out.csv')
...
larvae <- read_csv('data/bellevue larval index2001_2018.csv')
colnames(larvae)<-c("year","surface_tows","log_surface_tows")
glimpse(larvae)
larvae$surface_tows_lag2 <- lag(larvae$surface_tows, 2)
#View(larvae)

## Pseudocalanus data (1999-2016)----
# source "Copy of Copy of PSEUSP27_1999_2016.xlsx" obtained from Gary Maillet
pscal <- read_csv("data/pseudocal_1999_2018.csv")
glimpse(pscal)
pscal$year
...
#add years
ps_tot[21,] <- NA
ps_tot[21, 1] <- matrix(c(2019), ncol = 1) 
...
#change name back to df for simplicity
df <- capelin_join
glimpse(df)
...
# this normalizes the explanatory variables but bc of the lag, the last two years drop off the bottom of the data set
df1 %<>%
     mutate_each_(funs(scale),cols)
glimpse(df1)

# due to EVs dropping off the bottom of the data set in the above line, this code normalizes the variables and the last two years can be copied to Line 52-65 above; used to generate the prediction interval
df1$meanCond 
df1$meanCond_lag
co_temp <- df1[c("year", "meanCond")]
co_temp$meanCond_n <- scale(co_temp$meanCond)
ps_temp <- df1[c("year", "ps_meanTot")] # copy last two values of ps_meanTot_n[,1]
ps_temp$ps_meanTot_n <- scale(ps_temp$ps_meanTot)
st_temp <- df1[c("year", "surface_tows")] # grab last 2 of surface_tows_n[1]
st_temp$surface_tows_n <- scale(st_temp$surface_tows)
temp <- cbind(co_temp, ps_temp, st_temp)
head(temp)
temp<-temp[,-c(4,7)] #what is this for?
tempg<-gather(temp, "measure","value",-year) %>% filter(!(measure %in% c("meanCond", "ps_meanTot", "surface_tows")))

# this graph shows the trends for the EV's on normalized scale 
ggplot(tempg,aes(x=year,y=value, colour=measure) )+geom_line()

# divide by 100
df1$tice <- df1$tice/100  # copy last 2 values of this for tice
mean(df1$tice)
...
##########################################################################
########### Done loading data, starting to look at correlations ##########
##########################################################################
...
# print(w_r1$pareto_k)
# compare(w_r1, w_m1)
# note that we tried to do a WAIC approach but teh parento K were poor
...
# calculate Bayes R-squared.  See Gelman et al. 2019.
...
p1 <- postPriors(df = alpha$df, df2 = prior, df3 = alpha$df_cred, limits, x_label, priormean, priorsd, by_bin = .1)
...
#gamma tice-width
prior <- runif(n = 10000, 0, 3.65)*100
limits <- c(min(gamma$df)-0.3, max(gamma$df) + 0.3)
x_label <- "gamma"
bin_1 <- mean(gamma$df)/100
...
# An analysis of the sensitivity of the models to values of the input values in the penultimate year, i.e. the biomass estiamte for teh current calendar year with RAP in Feb/Mar and fishing in summer, ergo the first predicted value 
# sets the values for the predition interval
...
# the values TIpred and COpred are from lines 46-48, ie values for 2019
...
num_forecasts = 1 # this is just for 2019
# 2 extra years
...
for(i in 1:13){ # 13 is the number of scenarios being tested, not years of data
  message("i = ",i)
...
per_10 <- rep(NA, 13)
per_90 <- rep(NA, 13)
pi_tabl <- cbind(model, pi_pt, per_2_5, per_10, per_90, per_97_5)
# model being 13 long creates the warning message here
...
# loop to collect all of the prection intervals for last year (2019)
for(i in seq(model)){
     pi_tabl[i, 2] <- p_med_[[i]][17]
     pi_tabl[i, 3:6] <- pi_df3_[[i]][,17]
}
...
if(scale == "ln"){
     ...
     pi_tabl$per_10 <- as.numeric(levels(pi_tabl$per_10)[pi_tabl$per_10])
     pi_tabl$per_90 <- as.numeric(levels(pi_tabl$per_90)[pi_tabl$per_90])
...
# graph summarizing the influence of the knockouts on 
#TI values first, Cond values second
...
p <- p + geom_errorbar(aes(ymax = per_90, ymin = per_10), width = 0.5)
...
# This shows the reason why the capelin biomass for middle values of tice are slightly higher than the high values.  Also shows the influence of high and low values on model
# temp is tice values from 2003-2018
#y_med is capelin biomass for 1 = model, 5 = MH, 9 = LL
legend <- c("model", "MH", "LL")

p3 <- ggplot()
p3 <- p3 + geom_line(aes(x = c(temp[5:21]), y = y_med_[[1]]))
p3 <- p3 + geom_line(aes(x = c(temp[5:21]), y = y_med_[[5]]), colour = "red")
p3 <- p3 + geom_line(aes(x = c(temp[5:21]), y = y_med_[[13]]), colour = "blue")
p3 <- p3 + theme_bw(base_size = 20) + theme(plot.margin = unit(c(0.5, 1, 0.5, 0.5), "cm"))
p3 <- p3 + xlab("tice/100")
p3 <- p3 + ylab("capelin biomass")

p3
ggsave(paste0("Bayesian/", filepath, "/tice_sensitivity.pdf"), width=10, height=8, units="in")
